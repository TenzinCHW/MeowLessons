...
class MySMClass(sm.SM):
    ...
    def getNextValues(self, state, inp):
        left, fleft, front, fright, right, back = inp.sonars[:6] #fright and right sensor positions are swapped right now
        print inp.sonars
        if state == 0: #Head Straight until find wall
            print "state 0"
            if front <=0.3 or fright <= 0.6:
                nextstate = 1
                return (nextstate, io.Action(fvel=0.2, rvel=0.2))
            elif fright > 0.5 and right < 0.5:
                nextstate = 2
                return (nextstate, io.Action(fvel=0.2, rvel=-0.2))
